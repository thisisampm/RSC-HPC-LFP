...
for isubject = 1:length(file_list_subjects)
    ...
    for isession = length(file_list_sessions)-3 : length(file_list_sessions)
        ...
        %save
        save(full_save_file, 'clusters', 'csc_mtx', 'datamtx', 'spike_waveforms');
    
        %clear
        clearvars('clusters', 'csc_mtx', 'datamtx', 'spike_waveforms');
