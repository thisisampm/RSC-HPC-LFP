function [wt,p,coi,freq] = cwt_ampm(x,varargin)
...
if istimetable(x)
    ...
    % If RowTimes are a datetime array, convert to a duration array for
    % further processing.
    if isdatetime(TTable.SampleTimes)
        TTable.SampleTimes = TTable.SampleTimes-TTable.SampleTimes(1);
    end
...
if isTimeTable
    % A TimeTable specifed without a SamplingFrequency or
...
if ~antiAnalytic
    ...
    hcoi = plot(AX,t,coitmp,'w--','linewidth',2);
    hcoi.PickableParts = 'none';
...
elseif antiAnalytic
    ...
    hcoi1 = plot(AX(1),t,coitmp,'w--','linewidth',2);
    hcoi1.PickableParts = 'none';
    ...
    hcoi2 = plot(AX(2),t,coitmp,'w--','linewidth',2);
    hcoi2.PickableParts = 'none';
...
hdcm.UpdateFcn =  {@cwtPerCursorUpdateFunction, t, period};
...
elseif antiAnalytic
    ...
    xlbl = [getString(message('Wavelet:wcoherence:Time'))  ' (' ut ')'];
    titleString = {getString(message('Wavelet:cwt:ScalogramTitle'));...
        getString(message('Wavelet:cwt:ScalogramTitlePos'))};
    titleString2 = getString(message('Wavelet:cwt:ScalogramTitleNeg'));
    ...
    hcoi1 = plot(AX(1),t,invcoi,'w--','linewidth',2);
    hcoi1.PickableParts = 'none';
    ...
    hcoi2 = plot(AX(2),t,invcoi,'w--','linewidth',2);
    hcoi2.PickableParts = 'none';
...
hdcm = datacursormode(hf);
hdcm.UpdateFcn =  {@cwtFreqCursorUpdateFunction, t, freq};
hdcm.SnapToDataVertex = 'off';
...
output_txt{1} = ['Time: ', num2str(thisTime, 4)];
output_txt{2} = ['Period: ', num2str(thisPeriod, 4)];
cVal = event_obj.Target.CData(yIdx, xIdx);
output_txt{3} = ['Magnitude: ', num2str(cVal)];
...
output_txt{1} = ['Time: ', num2str(thisTime, 4)];
output_txt{2} = ['Frequency: ', num2str(thisFreq, 4)];
cVal = event_obj.Target.CData(yIdx, xIdx);
output_txt{3} = ['Magnitude: ', num2str(cVal)];
